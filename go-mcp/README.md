...
MCP server exposing the FreeBirds SDK as tools, built on the
...
| Tool | Args | Returns |
|------|------|---------|
| `free-birds_list` | `entity`, optional `query` map | First page of records as JSON |
| `free-birds_load` | `entity`, `query` (e.g. `{id:N}`) | Single record as JSON |
...
After install, restart Claude Code; the `free-birds_list` and
`free-birds_load` tools become available in new sessions.
