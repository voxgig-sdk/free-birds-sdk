...
[MCP](https://modelcontextprotocol.io) server exposing the FreeBirds SDK as
two agent tools — `free-birds_list` and `free-birds_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-birds-mcp)
make build

# 2. Provide credentials via the environment
export FREE_BIRDS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user free-birds \
  -- /absolute/path/to/free-birds-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./free-birds-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// free-birds_list: first page of records
{ "entity": "bird" }
{ "entity": "bird", "query": { } }

// free-birds_load: one record by id
{ "entity": "bird", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-birds-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user free-birds \
     -- "$PWD"/dist/*/free-birds-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `free-birds_list` and `free-birds_load` tools now appear
   in new sessions. Ask the agent to *"list bird using free-birds"*
   and it calls `free-birds_list` with `{"entity":"bird"}`.
...
```sh
export FREE_BIRDS_APIKEY=sk_live_xxx            # API key
export FREE_BIRDS_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `free-birds_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `free-birds_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `FREE_BIRDS_APIKEY` | API key sent with every request. |
| `FREE_BIRDS_BASE` | Optional override of the API base URL. |
...
```sh
./free-birds-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"free-birds_load","arguments":{"entity":"bird","query":{"id":1}}}}'
```
