...
AQL-driven command-line client **and** interactive REPL for the FreeBirds
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/free-birds-cli)
make build
...
./free-birds-cli list bird
./free-birds-cli load 1 bird            # {id:1} shorthand
./free-birds-cli load '{id:1}' bird       # explicit match map

# 5. Override the API base URL for a single call
FREE_BIRDS_BASE=https://api.example.com ./free-birds-cli list bird

# 6. No arguments -> interactive REPL
./free-birds-cli
free-birds> list bird
free-birds> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/free-birds-cli
   ```
...
```sh
./free-birds-cli load 1 bird          # scalar shorthand for {id:1}
./free-birds-cli load '{id:1}' bird     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `free-birds>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an AQL atom (e.g. `bird`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/free-birds-cli`. |
...
(`./free-birds-cli <expr>`) and interactively (the REPL), and expressions compose the
