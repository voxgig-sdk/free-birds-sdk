...
class BirdEntity
  ...
  def make
    opts = @_entopts.dup
    BirdEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = FreeBirdsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Bird, Hash] the current Bird data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = FreeBirdsHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Bird fields)
  def match_get
  ...
  # Load a single Bird.
  #
  # @param reqmatch [BirdLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Bird.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Bird, Hash] the loaded Bird; raises FreeBirdsError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = FreeBirdsHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [BirdListMatch, Hash, nil] match filter (any subset of
  #   Bird fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Bird>, Array] the matching Bird items; raises FreeBirdsError on failure
