...
module FreeBirdsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = FreeBirdsResponse.new({})
    ...
    if fetch_err
      ...
      response = FreeBirdsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = FreeBirdsResponse.new(fetched)
    else
