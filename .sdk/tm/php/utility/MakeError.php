...
class FreeBirdsMakeError
{
    public static function call(?FreeBirdsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new FreeBirdsContext([], null);
        }
        $op = $ctx->op ?? new FreeBirdsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new FreeBirdsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof FreeBirdsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new FreeBirdsError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof FreeBirdsError) {
