...
class FreeBirdsTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            FreeBirdsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = FreeBirdsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = FreeBirdsTestRunner.getenv("PROJECTNAME_TEST_LIVE")
        override = FreeBirdsTestRunner.getenv("PROJECTNAME_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = FreeBirdsTestRunner.getenv(key)
        ...
        explain = FreeBirdsTestRunner.getenv("PROJECTNAME_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if FreeBirdsTestRunner._test_control is not None:
            return FreeBirdsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                FreeBirdsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            FreeBirdsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = FreeBirdsTestRunner.load_test_control()
...
def load_env_local():
    FreeBirdsTestRunner.load_env_local()


def env_override(m):
    return FreeBirdsTestRunner.env_override(m)


def entity_list_to_data(lst):
    return FreeBirdsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return FreeBirdsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return FreeBirdsTestRunner.load_test_control()


def live_delay_ms():
    return FreeBirdsTestRunner.live_delay_ms()
