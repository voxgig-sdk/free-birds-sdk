...
func TestBirdDirect(t *testing.T) {
	t.Run("direct-list-bird", func(t *testing.T) {
		setup := birdDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-bird", _mode); _shouldSkip {
			if _reason == "" {
	...
	t.Run("direct-load-bird", func(t *testing.T) {
		setup := birdDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-bird", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
type birdDirectSetupResult struct {
	client *sdk.FreeBirdsSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func birdDirectSetup(mockres any) *birdDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["FREEBIRDS_TEST_BIRD_ENTID"]; ok {
		...
		return &birdDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &birdDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
