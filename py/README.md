...
The Python SDK for the FreeBirds API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Bird(None).list(None, None)
if err:
    raise Exception(err)
...
```python
result, err = client.Bird(None).load({"id": "example_id"}, None)
if err:
    raise Exception(err)
print(result)
```
...
result, err = client.FreeBirds(None).load(
    {"id": "test01"}, None
)
