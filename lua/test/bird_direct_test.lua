...
describe("BirdDirect", function()
  it("should direct-list-bird", function()
    local setup = bird_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-bird", setup.live and "live" or "unit")
  ...
  it("should direct-load-bird", function()
    local setup = bird_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-bird", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function bird_direct_setup(mockres)
