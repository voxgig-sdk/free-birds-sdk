# FreeBirds Lua SDK Reference

Complete API reference for the FreeBirds Lua SDK.
...
```lua
local results, err = client:Bird(nil):list(nil, nil)
```
...
```lua
local result, err = client:Bird(nil):load({ id = "bird_id" }, nil)
```
