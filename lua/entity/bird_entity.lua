...
function BirdEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, BirdEntity)
  self._name = "bird"
  self._client = client
...
function BirdEntity:get_name()
  return self._name
end


function BirdEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return BirdEntity.new(self._client, opts)
end


function BirdEntity:data_set(args)
  if args ~= nil then
...
function BirdEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function BirdEntity:match_set(args)
  if args ~= nil then
...
function BirdEntity:stream(action, args, callopts)
...
function BirdEntity:_run_op(ctx, post_done)
  local utility = self._utility
