...
- Releases: [https://github.com/voxgig-sdk/free-birds-sdk/releases](https://github.com/voxgig-sdk/free-birds-sdk/releases)
...
```ts
try {
  const bird = await client.Bird().load({ id: 'example_id' })
  console.log(bird)
} catch (err) {
...
const bird = await client.Bird().load({ id: 'test01' })
...
| `tester(testopts?, sdkopts?)` | `FreeBirdsSDK` | Create a test-mode client instance. |

#### Static methods

| Method | Returns | Description |
| --- | --- | --- |
| `FreeBirdsSDK.test(testopts?, sdkopts?)` | `FreeBirdsSDK` | Create a test-mode client. |
...
| `client` | `client(): FreeBirdsSDK` | Return the parent SDK client. |
...
free-birds/
├── src/
│   ├── FreeBirdsSDK.ts        # Main SDK class
