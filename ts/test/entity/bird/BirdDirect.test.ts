...
describe('BirdDirect', async () => {
  ...
  test('direct-load-bird', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-bird', setup.live)) return
  ...
  test('direct-list-bird', async (t: any) => {
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-bird', setup.live)) return
    const { client, calls } = setup
