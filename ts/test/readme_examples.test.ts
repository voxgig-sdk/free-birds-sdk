...
//     (\`new FreeBirdsSDK(...)\` / \`FreeBirdsSDK.test(...)\` become
//     \`FreeBirdsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"bird":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new FreeBirdsSDK(...)\` and any
// \`FreeBirdsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+FreeBirdsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/FreeBirdsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
