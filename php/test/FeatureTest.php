...
class FtCtrl extends FreeBirdsControl
{
...
class FtClient
{
    ...
    private FreeBirdsUtility $utility;

    public function __construct(FreeBirdsUtility $utility, array $options)
    {
    ...
    public function get_utility(): FreeBirdsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (FreeBirdsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public FreeBirdsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (FreeBirdsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = FreeBirdsConfig::make_feature($name);
    ...
    private static function buildUrl(FreeBirdsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof FreeBirdsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new FreeBirdsResponse($fetched);
        }
    ...
    private function fail(FreeBirdsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (FreeBirdsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
