...
class BirdEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = FreeBirdsSDK::test($seed, null);
        $seen = iterator_to_array($base->Bird(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = FreeBirdsConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = FreeBirdsSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Bird(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set FREEBIRDS_TEST_BIRD_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function bird_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("FREEBIRDS_TEST_BIRD_ENTID");
    ...
    if ($env["FREEBIRDS_TEST_LIVE"] === "TRUE") {
        ...
        $client = new FreeBirdsSDK(Helpers::to_map($merged_opts));
    }
